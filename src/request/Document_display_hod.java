 ...
 * @author RAJIL KV
 */
class Document_display_hod extends JFrame{
    public ResultSet res;
    public JTable table ;
    JLabel jl;

    public Document_display_hod() {
    setSize(1000,600);
    ...
    private final Action back = new AbstractAction()
    {
        ...
        public void actionPerformed(ActionEvent e)
        {   setVisible(false);
            Hod_view_Request std = new Hod_view_Request();
            std.setVisible(true);
    ...
    public static void main(String[] args) throws SQLException {
        Document_display rd = new Document_display();
    }

    private void Display() {
        MyModel_7 data = new MyModel_7();
  ...
  public void mouseClicked(MouseEvent e) {
    if (e.getClickCount() == 1) {
      ...
      int row = target.getSelectedRow();
        try {
            ((MyModel_7) table.getModel()).deleteRow(row);
        } catch (SQLException ex) {
            Logger.getLogger(Document_display.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
  }
});
